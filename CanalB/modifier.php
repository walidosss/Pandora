<?php 
$idUser =$_SESSION['S_LOGIN']['id'];
$query = "select *, DATE_FORMAT(date_de_naissance, '%d%m%Y') as myDate from utilisateur where id=".$idUser;
$connect = connectDB();
...
<hgroup class="page-head">    		
	<h2>Modifier <span>Profil</span></h2>
...
<div id="container" class="clearfix">
	...
	<div id="content">
		...
		<div class="contact-form-container">
			<h3 class="smart-head">Edition</h3>
			<p>Compiler le formulaire suivant pour vous inscrire.</p>
			<form action="submit.php" id="contact-form" method="post" novalidate>
			<!--form action="submit.php" id="save-form" method="post" novalidate-->
				<div class="form-cell">
					<label class="display-ie8" >Nom:</label>
					<input type="text" placeholder="Nom" class="name required" id="nom" name="nom" title="Veuillez saisir votre nom"  value="<?php echo $line['nom'];?>"/><span>*</span>
				</div>
				
				<div class="form-cell">
					 <label class="display-ie8" >Prénom:</label>
					<input type="text" placeholder="Prénom" class="phone required" id="prenom" name="prenom" title="Veuillez saisir votre prénom" value="<?php echo $line['prenom'];?>" /><span>*</span>
				</div>
				
				<div class="form-cell">
					 <label class="display-ie8" >Login:</label>
					<input type="text" placeholder="Login" class="phone required" id="login" name="login" title="Veuillez saisir votre login" value="<?php echo $line['login'];?>" readonly/><span>*</span>
				</div>
				
				<div class="form-cell">
					 <label class="display-ie8" >Email:</label>
					<input type="text" placeholder="Email" class="email required" id="email" name="email" title="Veuillez saisir votre email" value="<?php echo $line['email'];?>" /><span>*</span>
				</div>
				
				<div class="form-cell">
					 <label class="display-ie8" >Date de naissance:</label>
					<input type="text" placeholder="Date de naissance" class="subject required" id="naissance" name="naissance" title="Veuillez introduire votre date de naissance" value="<?php echo $line['myDate'];?>" /><span>*</span>
				</div>
				<div class="form-cell">
					 <label class="display-ie8" >Mot de passe:</label>
					<input type="password" placeholder="Mot de passe" class="subject required" id="password1" name="password1" title="Veuillez saisir votre mot de passe" value="<?php echo $line['mot_de_passe'];?>"/><span>*</span>
				</div>  
				<!--div class="form-cell">
					 <label class="display-ie8" >Retapez le mot de passe:</label>
					<input type="password" placeholder="Retapez le mot de passe" class="subject required" id="password2" name="password2" title="Veuillez retaper votre mot de passe" /><span>*</span>
				</div-->													
				
				<div class="form-row">													
					<input type="submit" name="submit" value="Modifier" class="submit readmore"/>
					<!--input type="submit" id="nl_submit" value="Inscrire" class="readmore"-->
				</div>
				<input type="hidden" name="idUser" value="<?php echo $_SESSION['S_LOGIN']['id']?>">
